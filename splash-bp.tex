...
\begin{tightdescription}
   ...
   same thread. Nodes are collected into list \texttt{L} and
   appended to list \texttt{Next} (line 21).
   
   \item[First phase]: When the number of nodes in the tree reaches a certain
   limit, a \texttt{first-phase} (lines 11-12) is generated to update the beliefs of
   all nodes in the tree. As the nodes are updated, starting from the leaves and
   ending at the root, an \texttt{update} fact is derived to update the belief
   values (line 35).
...
\begin{dblfig}
   \begin{center}
      \subfloat[]{\includegraphics[width=5.5cm]{results/system_belief-propagation-400.png}}
      \subfloat[]{\includegraphics[width=5.5cm]{results/system_splash-bp-400.png}}
      \subfloat[]{\includegraphics[width=5.5cm]{results/system_improve_belief-propagation-400.png}}
   \end{center}
